import { addDailyArticleUsingPost } from '@/services/backend/dailyArticleController';
...
interface Props {
  ...
  onSubmit: (values: API.DailyArticleAddRequest) => void;
  onCancel: () => void;
}
 ...
 * @param fields
 */
const handleAdd = async (fields: API.DailyArticleAddRequest) => {
  const hide = message.loading('正在创建');
  try {
    await addDailyArticleUsingPost(fields);
    hide();
...
const CreateModal: React.FC<Props> = (props) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // 处理表单数据
      const postData: API.DailyArticleAddRequest = {
        ...values,
        publishDate,
      };
